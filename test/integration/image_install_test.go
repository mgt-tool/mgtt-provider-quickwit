...
const (
	imgLocalTag    = "mgtt-provider-quickwit-it:test"
	providerName   = "quickwit"
	expectCaps     = ""
	expectNetwork  = "host"
	registryPort   = "15805"
	registryName   = "mgtt-provider-quickwit-it-registry"
	localPushTag   = "localhost:15805/mgtt-provider-quickwit:it"
)
...
func TestImageInstall_Capabilities(t *testing.T) {
	...
	digest := fetchManifestDigest(t, registryPort, "mgtt-provider-quickwit", "it")
	if digest == "" {
		t.Fatal("could not resolve manifest digest after push")
	}
	digestRef := fmt.Sprintf("localhost:%s/mgtt-provider-quickwit@%s", registryPort, digest)
